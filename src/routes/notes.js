    ...
    if (!inicio) {
        ...
        horaInicio = null;
         ...
         if (!dia) {
            ...
            var hoy = moment(new Date()).subtract(3, 'hours');;
            console.log(hoy)
            console.log(localLocale)
            if (hoy.isSame(localLocale, 'day')) {
                var diadesemana= "Hoy"   
...
router.post('/notes/new-note/hoy', isAuthenticated, async (req,res)=>{
    ...
    if (horario) {  
       ...
       mensaje = null;
        ...
        var diadesemana = "Hoy";
        const newNote = new Note({title, dia: hoy, date, mensajeReides, horaInicio, horaFin, mensaje, diadesemana})
        newNote.user = req.user.id; 
        await newNote.save()
